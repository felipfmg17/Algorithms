...
class UF{
	...
	void joinSets(int a,int b){
		if(!isSameSet(a,b)){
			...
			if(r.get(x)>r.get(y))	{	p.set(y,x);	l.set(x,l.get(x)+l.get(y));	}
			else{
				p.set(x,y);
...
/*
	class UF: Union Find
		Attributes
			List<Integer> p,r,l 		--- p: parents, r: rank, l=length
			int size,sets				--- number of individual elements, number of sets
